...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
void calc_next(char cur[], char next[])
{
	int last = 0;
	for (int i = N_NUM; i >= 1; i--) {
		if (cur[i] != 0) {
...
{
    cout << "请输入一个二进制纯小数，小数点开头，小数点后不超过" << N_POW << "位(例：.101101)" << endl;
    while (1) {
        cin >> pure_decimal;
        int invalidcnt = 0; // 移到循环内部，确保每次输入都重置
        ...
        if (pure_decimal[0] != '.') {
            ...
            if (bit_length == 0 && pure_decimal[1] == '\0') {
                cout << "小数点后不能为空，请重输" << endl;
                invalidcnt++;
            }
	    else {
                for (int i = 1; pure_decimal[i] != '\0'; ++i) {
...
int main()
{
	...
	*  2、第[0]列存放幂次，[1]~[112]列依次存放十分位、百分位、千分位、...  
	  （2^-i次方的十进制小数，最多i位），[113]预留，用于某些特殊要求，比如\0
	...
	*		第[112]行 -112   ...
	*
	注：1、本二维数组的类型，既可以是int，也可以是char，因为计算时，每个元素的值都在0-9之间，用char足够了
	   （char型输出时注意数字0-9和字符0-9的问题即可）
		2、因为如此，本题放在数组综合训练中，而没有放在字符数组中
	*/
	char power_table[N_POW + 1][N_NUM + 1] = { 0 };
